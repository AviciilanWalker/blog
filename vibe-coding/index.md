...
Vibe Coding 的本质是：**你用自然语言描述想要什么，AI 生成代码，你审查结果，不行就继续描述、让 AI 改，直到满意。** 无需逐行敲代码，只用像一个产品经理在跟程序员沟通——只不过这个程序员是 AI，而且响应速度以秒为单位。
...
**关键原则：你永远是最后一道关。** AI 生成的代码你敢直接上生产，那就叫赌博了。Vibe Coding 的正确姿势是"AI 写初稿，你来把关"。
...
目标：在 Windows 上装好 Claude Code，通过 CC Switch 接入 DeepSeek（便宜且国内直接用），然后用它做一个小项目。
...
下载完后双击 `.msi` 文件，一路点"下一步"装好。桌面会出现 **CC Switch** 图标。

---

### 第四步：获取 DeepSeek API Key

API Key 就像是一把"钥匙"，证明你有权限使用 DeepSeek 的模型。

1. 打开浏览器，访问 **[platform.deepseek.com](https://platform.deepseek.com)**，注册一个账号（邮箱就行）
2. 登录后在左侧菜单找到 **API Keys**（API 密钥）
3. 点 **创建新的 API Key**，起个名字比如"claude-code"，然后点创建
4. **立刻复制那一串 `sk-` 开头的字符！** 关闭后就再也看不到了

DeepSeek 按使用量收费，但新用户有免费额度，够你试用一阵子了。后面觉得好用再充值，最低充 2 美元。
...
2. 顶部有一排图标，点最左边那个 **Claude** 图标（一个类似无穷符号 ∞ 的图标）
3. 点右上角的 **+** 按钮，在供应商列表里找到 **DeepSeek**，点击选中
4. 把刚才复制的 API Key 粘贴到 **API Key** 输入框里
5. 确认 **模型** 那里默认就有 `deepseek-v4-flash`（如果需要在右侧填入模型名）
6. 点 **添加** 保存
...
先试试它是不是真的在走 DeepSeek：

```
你是什么模型？请如实回答。
```

如果回答里提到 DeepSeek，说明配置成功！
