...
- [目录](#目录)
- [概览：模型训好了，然后呢？](#概览模型训好了然后呢)
- [4.1 API 调用：把 AI 嵌入你的程序](#41-api-调用把-ai-嵌入你的程序)
    - [2026 年主流 API 提供商](#2026-年主流-api-提供商)
    - [核心参数详解](#核心参数详解)
        - [temperature（温度，0~2，默认 1）](#temperature温度02默认-1)
        - [top\_p（核采样，0~1，默认 1）](#top_p核采样01默认-1)
        - [max\_tokens（最大输出长度）](#max_tokens最大输出长度)
        - [stop sequences（停止符）](#stop-sequences停止符)
        - [system prompt vs user prompt](#system-prompt-vs-user-prompt)
    - [流式输出（Streaming）](#流式输出streaming)
    - [Token 计费](#token-计费)
    - [动手：命令行 AI 翻译工具](#动手命令行-ai-翻译工具)
- [4.2 Prompt Engineering（提示工程）](#42-prompt-engineering提示工程)
