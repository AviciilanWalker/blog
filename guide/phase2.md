...
"中国的首都是" 
    │
    ▼
① Tokenization ─── 切成词元: ["中国", "的", "首都", "是"]
    │
    ▼
② Embedding ────── 每个词元变成一个高维向量
    │               "中国" → [0.23, -0.45, 0.78, ...]（4096 个数字）
    ▼
③ + 位置编码 ───── 加上位置信息（"中国"在第 1 位，"首都"在第 3 位）
    │
    ▼
④ Self-Attention ─ 每个词元"看"其他所有词元，理解上下文
    │               "苹果"在"吃苹果"和"苹果手机"里含义不同，就是这步解决的
    ▼
⑤ Feed-Forward ─── 对每个词元的表示做非线性变换，提炼特征
    │
    ▼
⑥ ④+⑤ 重复 N 层 ── 层层提炼，越来越"懂"语义
    │
    ▼
⑦ 输出层 ───────── 预测下一个词元的概率分布
    │                "北京" 91.2%, "上海" 3.5%, "广州" 1.8% ...
```
...
输入 x: [1, 4096]
       │
       ├──→ W_gate [4096, 14336] ──→ SiLU ──→ [1, 14336]  (门控系数)
       │                                          │
       └──→ W_up   [4096, 14336] ──────────→ [1, 14336]  (信息候选)
                                                  │
                                          逐元素相乘 ⊙
                                                  │
                                            [1, 14336]
...
原始文本
    │
    ▼
① Tokenization ──── BPE 算法：高频字符对反复合并
    核心直觉:      词元 ≠ 单词，是统计意义上的"常用片段"
    │
    ▼
② Embedding ────── 查表：ID → 高维向量
    核心直觉:      向量空间里，语义相近的词真的挨得近
    │
    ▼
③ + 位置编码 ───── 正弦波 / RoPE 旋转
    核心直觉:      Attention 不感知顺序，必须手动注入位置
    │
    ▼
④ Self-Attention ─ Q·K 算相关 → Softmax 归一化 → ×V 加权融合
    核心直觉:      每个词问"我该关注谁？"，用别人的信息来更新自己
    │
    ▼
⑤ Multi-Head ───── 多套 Q/K/V 并行，各学各的关系
    核心直觉:      一个头学语法，一个头学指代，一个头学语义……组合起来才是完整理解
    │
    ▼
⑥ FFN ──────────── 两层全连接，中间膨胀 4-8 倍
    核心直觉:      词元之间"交流"完（Attention）之后，各自"消化"（FFN）
    │
    ▼
⑦ 残差 + LN ────── 输入抄近道加到输出，再归一化
    核心直觉:      没有残差连接，深层网络根本训不动
    │
    ▼
⑧ 第④→⑦ 重复 N 层 ─ 层层提炼
    核心直觉:      底层学语法，中层学语义，高层学推理
    │
    ▼
⑨ 输出投影 ─────── 最后一层的向量 × 嵌入矩阵的转置 → 词表大小的概率分布
    核心直觉:      "在 50000 个候选 token 中，下一个最可能是哪个？"
    │
    ▼
⑩ 采样 ──────────── Temperature + Top-p + Top-k
    核心直觉:      不是每次都选最高概率的，加点随机性，文本才像"人写的"
    │
    ▼
⑪ 自回归循环 ───── 生成的 token 拼回输入，再来一轮
    核心直觉:      LLM 的"逐字输出"根源——每生成一个字，就是在给下一个字出题
```
